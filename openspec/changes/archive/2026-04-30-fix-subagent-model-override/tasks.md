...
- [x] 1.1 确认 `pi-subagents` 的 `subagents.agentOverrides` 机制支持 `model` 字段覆盖
- [x] 1.2 确认 7 个受影响 builtin agent 的名称和当前模型（`openai-codex/*`）
...
- [x] 3.1 重新加载 `/reload` 使 settings 变更生效
- [x] 3.2 使用 `dispatch` 测试至少一个受影响 agent（如 `researcher`），确认不再报 "No API key found"
