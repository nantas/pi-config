...
- [x] 2.1.1 移除 `CustomEditor` import 和 `DollarSkillEditor extends CustomEditor` 类定义
  - 覆盖 spec: `dollar-skill-autocomplete` REMOVED "Editor Auto-Trigger on $"
  - 验证: 文件中无 `CustomEditor` 引用、无 `DollarSkillEditor` 类

- [x] 2.1.2 移除 `ctx.ui.setEditorComponent(...)` 调用
  - 覆盖 spec: `dollar-skill-autocomplete` REMOVED "Editor Auto-Trigger on $"
  - 验证: 文件中无 `setEditorComponent` 调用

- [x] 2.1.3 将 `pi.on("input", handler)` 从 `session_start` 回调内**提到顶层**（`export default function` 中），确保 handler 只注册一次
...
- [x] 2.1.4 保持 `ctx.ui.addAutocompleteProvider(...)` 包装和 `session_start` handler 不变
...
- [x] 2.2.1 更新 `openspec/specs/dollar-skill-autocomplete/spec.md`：移除 `Editor Auto-Trigger on $` requirement block，替换 `Dollar-Prefixed Skill Completion` 的 WHEN/THEN 改为 Tab 触发语义
...
- [x] 2.2.2 更新 `openspec/specs/dollar-skill-invoke/spec.md`：新增 `Input Handler Single Registration` requirement
  - 覆盖 spec: `dollar-skill-invoke`（同步 delta → main）
  - 验证: main spec 中包含 handler 注册约束的 requirement

### 2.3 新增参考文档

- [x] 2.3.1 创建 `docs/reference/pi-extension-editor-conflict.md`，包含：
...
- [x] 2.4.1 运行 `scripts/sync-pi-agent.sh` 同步修改到 `~/.pi/agent/extensions/dollar-skill-invoke.ts`
  - 验证: 文件同步后，启动 Pi session 确认无加载错误

## 3. 收敛与验证准备

- [x] 3.1 功能验证检查点：
  - [x] `pi -e .pi/extensions/dollar-skill-invoke.ts` 启动无错误
   - [x] 输入 `$` 按 Tab 显示 skill 自动补全
  - [x] 提交 `$skill-name` text 展开为 `<skill>` block
  - [x] `/` 自动补全无 `skill:xxx` 条目
  - [x] `/new` 后 `$` Tab 补全正常工作（验证 handler 不累积）
  - [x] `/new` 后 `$skill-name` 展开正常工作
- [x] 3.2 热重载验证：`/reload` 后功能正常
- [x] 3.3 冲突恢复验证：在启用 `pi-powerline-footer` 的情况下，`$` Tab 补全和 `$skill-name` 展开均正常工作
