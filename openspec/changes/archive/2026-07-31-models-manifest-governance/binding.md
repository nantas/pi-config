...
- `additional_context_refs`: `repo://orbitos/20_项目/Pi_Config/项目进度总览.md`
...
- 项目主页保留阶段摘要与入口，进度页承接本次规划结论与后续专题链接
- 当前仓库中的研究文档可作为实现前参考输入，但不得替代 change artifacts 的约束效力
...
- [x] 已确认 models.json 治理边界：manifest 声明的 provider 为权威，未声明 provider 保留目标端值（与 settings.json 同款 preserve 语义）
