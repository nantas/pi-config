...
Provide a planner mode toggle via `Ctrl+Alt+P` shortcut and `/planner` command that switches the active model to `deepseek/deepseek-v4-pro` with read-only tool restrictions, with state persistence, UI status indicator, and stale context cleanup.
