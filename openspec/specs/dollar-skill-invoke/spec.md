...
Defines the behavior for expanding `$skill-name` tokens into `<skill>` blocks on user input submission. The system intercepts the `input` event and transforms the text before it reaches the LLM pipeline.
