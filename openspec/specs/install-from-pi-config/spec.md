...
Define a cross-repository capability installer skill that reads the pi-config catalog from `~/.pi/agent/catalog/pi-config.yaml` and installs skills, extensions, or packages into the current repository's `.pi/` directory.
