...
| **技能** | 4 | `install-from-pi-config`、`notion`、`agent-reach`、`worktrunk-isolation` |
...
| **模型配置** | 1 | `deepseek`（deepseek-v4-flash 走 Responses API，渲染到 `~/.pi/agent/models.json`） |
| **外部包** | 12 | `pi-tab-status`（tab 状态图标）、`pi-subagents`（子代理系统）、`pi-powerline`（powerline 状态栏）、`pi-terminal-signals`（终端信号）、`pi-tool-display`（工具调用可视化）、`pi-mcp-adapter`（MCP 适配器）、`pi-question`（交互提问）、`pi-webfetch`（网页抓取）、`pi-fff`（模糊文件查找）、`ponytail`（极简编码）、`pi-codex-image-gen`（图像生成）、`fusion-harness`（多模型编排） |
